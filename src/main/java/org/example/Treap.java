...
public class Treap {
    ...
    public void fillValueCase1(int a, int b, int value) {
        // [0 < a) [a < N)
        Node[] low = root.split(a );
        // [a < b) [b < N)
        Node[] high = low[1].split(b - a);

        high[0].fillToPromiseValue(value);
        root = merge(merge(low[0], high[0]), high[1]);
    }

    public void fillValueCase2(int a, int b, int value) {
        // [0 < a) [a < N)
        Node[] low = root.split(a);
        // [a < b) [b < N)
        Node[] high = low[1].split(b - a);

        // inorder + value на каждом следующем элементе
        inorderCase2(high[0], (long) value);
        root = merge(merge(low[0], high[0]), high[1]);
    }
    ...
    private Long inorderCase2(Node cur, Long add) {
        if (cur == null) {
            return add;
        }
        cur.pushPromise();
        add = inorderCase2(cur.left, add);
        cur.value = cur.value + add;
        add += add;
        add = inorderCase2(cur.right, add);
        add += add;
        return add;
    }
    ...
    public static class Node {
        ...
        public Node(Long value, long priority) {
            this(value, priority, null, null);
        }

        public Node(Long value, long priority, Node left, Node right) {
        ...
        public void recalculate() {
            ...
            setSumValue(sumValueOf(left) + sumValueOf(right) + (long) value);
        }

        private static Long valueOf(Node node) {
            return node != null ? node.value + node.addPromise : null;
        }

        private static Long minValueOf(Node node) {
            return node != null ? node.statistic.minValue : null;
        }

        private static Long maxValueOf(Node node) {
        ...
        public void fillToPromiseValue(long add) {
            addPromise = add - value;
            recalculate();
        }

        public void fillToPromiseValueCase2(long add) {
            addPromise = add - value;
            recalculate();
        }
